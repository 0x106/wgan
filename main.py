...
if torch.cuda.is_available():
	opt.cuda = True
	opt.path = '/output/'
	os.system("mkdir "+opt.path+"vault")
	opt.output_path = opt.path+"vault/"

else:
	index = 1
	while os.path.exists(opt.path+'vault/'+str(index).zfill(2)):
		index += 1
	os.makedirs(opt.path+'vault/'+str(index).zfill(2))
	opt.output_path = opt.path+'vault/'+str(index).zfill(2)
...
if opt.cuda:
    net.critic.cuda()
    net.generator.cuda()
    var.data = var.data.cuda()
    var.one, var.mone = var.one.cuda(), var.mone.cuda()
    var.noise, var.fixed_noise = var.noise.cuda(), var.fixed_noise.cuda()
